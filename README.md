...
db_gui は [CloudBeaver](https://dbeaver.com/docs/cloudbeaver/) で、アプリの DB への接続は [initial-data-sources.conf](.devcontainer/db_gui-container/initial-data-sources.conf) で登録済みです。docs は [Scalar の API リファレンス](https://scalar.com/products/api-references/integrations/docker)で、`make gen` が生成した OpenAPI ドキュメント（api/gen/openapi/）をマウントして `http://localhost:${DOCS_PORT}` に出します。api サーバを起動していれば、ページから各ルートにリクエストを送れます。送信先は docs 自身ではなく api のポートで、別オリジンからの呼び出しを api が許すための `CORS_ORIGINS` は docker-compose.yml が渡しています（[cors.go](api/internal/server/cors.go)）。
