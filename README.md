...
アプリはアカウント制です。web を開くと最初にログイン画面が出るので、「アカウントを作成する」からメールアドレスとパスワード（8 バイト以上 72 バイト以下）で登録してください。todo は登録したアカウントのものだけが見え、他人の todo は存在しないものとして扱われます（[auth.proto](proto/auth/v1/auth.proto)）。セッションの有効期間は 24 時間で、更新の仕組みは無いため切れたら再ログインになります。ログアウトすると、そのセッションはサーバ側で本当に閉じます。トークンは署名済みで取り消せないので、`sessions` テーブルの行の有無で判断する作りです（[verifier.go](api/internal/auth/verifier.go)）。行が消えれば、Cookie を消せないクライアントが持ち出したトークンも同時に通らなくなります。閉じるのは要求してきたセッションだけなので、同じアカウントの他の端末はログインしたままです。リクエストごとの照会は 1 クエリで、セッションの行を読むついでに `users` を突き合わせ、アカウントがまだ開いているかも同時に見ます（[sessions.sql](api/queries/sessions.sql)）。期限切れの行は、そのアカウントが次にログインしたときにまとめて捨てます。トークンの署名鍵 `TOKEN_SECRET` はローカル開発用の値を [docker-compose.yml](docker-compose.yml) が直接渡しています（db_gui の認証情報と同じ扱いです）。デプロイするなら各環境で自前の値に差し替えてください。

ブラウザは HttpOnly の session Cookie で認証されます（[cookie.go](api/internal/auth/cookie.go)）。ページのスクリプトからは読めないので、XSS があってもトークンを持ち出せません（開いている間に成りすまされるのは防げません）。スクリプトから消すこともできないため、ログアウトは `AuthService/LogOut` を呼んで Cookie を失効させ、併せてセッションの行を消す形にしています。Cookie に頼る以上 CSRF が問題になりますが、ハンドラが Connect の `Connect-Protocol-Version` ヘッダを必須にしているので（[server.go](api/internal/server/server.go)）、ヘッダを付けられない他サイトのフォームや、preflight を CORS に拒否されるスクリプトからは通りません。ブラウザ以外のクライアントは、応答に入っている `accessToken` を `Authorization: Bearer` で送る従来どおりの方法が使えます。Cookie には `Secure` が付き、http では送られません。ローカルは http なので [docker-compose.yml](docker-compose.yml) が `COOKIE_SECURE` を false にして外しています。api の既定は true で、https で配るデプロイでは何も設定しなくても付きます。api 自身は常に平文 http を喋るので（[h2c.go](api/internal/server/h2c.go)）、https で配るなら TLS はロードバランサなどの前段で終端してください。api が見るスキームは http のままですが、`COOKIE_SECURE` は独立した設定なので `Secure` は正しく付きます。

退会は todo 画面の一番下にある「退会する」から行います（`AccountService/DeleteAccount`、[account.proto](proto/account/v1/account.proto)）。セッションだけでは実行できず、パスワードをもう一度入力させます。取り消せない操作なので、借りられたセッションや席を立ったままのブラウザで使われないようにするためです。退会するとその場で全端末のセッションが消え、ログインもできなくなりますが、`users` の行は消さずに `deleted_at` を記録するだけです。データが実際に消えるのは 30 日後です。それまでのあいだは、運用側が `deleted_at` を null に戻せば必ず復旧できます。アプリから自分で復帰する導線はありません（退会したアカウントはログインできないため）。メールアドレスは退会しても解放されません。ユニーク制約が退会した行も数えるので（[schema.sql](api/schema.sql)）、同じアドレスで作り直せるのは 30 日を過ぎて行が消えた後です。猶予期間中に解放してしまうと、戻す先のアドレスを先に取られた退会者は復旧できなくなり、上の「必ず復旧できる」が成り立たなくなります。猶予期間中に同じアドレスで登録しようとしたときは、通常の重複と同じ `already_exists` を返します。退会済みだと区別して答えると、そのアドレスにアカウントがあったことを教えてしまうためです。

30 日を過ぎた行を実際に消すのは `api/cmd/purge` で、api 本体は何も定期実行しません。`make api-purge` で手元から実行でき、デプロイ先では cron や CronJob から同じコマンドを呼んでください。**これを回さない限りデータは消えません。** 消す対象は猶予期間を過ぎたものだけなので、実行頻度を上げても余分に消えることはなく、二重に走っても問題ありません。todo とセッションは `users` への外部キーが連れていきます。

認証まわりで手当てしていないものがあります。`SignUp` と `LogIn` はトークン無しで叩ける唯一の入口ですが、試行回数を数えてもいなければロックアウトもせず、レート制限も入れていません。アドレスが存在しない場合もダミーのハッシュと突き合わせて同じ時間をかける作りなので（[service.go](api/internal/auth/service.go)）、失敗しても 1 リクエストあたりの bcrypt の計算量は変わりません。つまりパスワードの総当たりが試し放題である点と、未認証のまま CPU を使わせられる点の両方が開いたままです。アカウント作成もメール検証が無く無制限に作れます。実運用に持っていくなら、前段のロードバランサで絞るか api にインターセプタを足すかを決める必要があります。
