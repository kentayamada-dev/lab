...
@test "drift がなく同じタイトルの open issue があれば、経緯コメントを付けて close する" {
  export DRIFT=false
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない')"
...
@test "同じラベルでもタイトルが完全一致しない issue は close しない" {
  export DRIFT=false
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない (2)' $'43\tchore: 別の課題')"
...
@test "同じタイトルの open issue が複数あっても、最初の1件だけ close する" {
  export DRIFT=false
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない' $'57\tchore: 設定が一致していない')"
...
@test "既存 issue の本文に同じハッシュがあれば、追記も作成もしない" {
  ...
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない')"
  export GH_ISSUE_VIEW_JSON="$(issue_view_json "本文 <!-- drift-hash: ${hash} -->")"
...
@test "既存 issue にハッシュが無ければ、コメントで追記する（作成はしない）" {
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない')"
  export GH_ISSUE_VIEW_JSON="$(issue_view_json "ハッシュの無い本文" "ハッシュの無いコメント")"
...
@test "本文のハッシュが一致しても、より後のコメントに別のハッシュがあれば追記する" {
  ...
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない')"
  other="$(printf '%064d' 0)"
  export GH_ISSUE_VIEW_JSON="$(issue_view_json "<!-- drift-hash: ${hash} -->" "<!-- drift-hash: ${other} -->")"
...
@test "本文のハッシュが違っても、最後のコメントのハッシュが一致すればスキップする" {
  ...
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない')"
  other="$(printf '%064d' 0)"
  export GH_ISSUE_VIEW_JSON="$(issue_view_json "<!-- drift-hash: ${other} -->" "ハッシュ無し" "<!-- drift-hash: ${hash} -->")"
...
@test "既存 issue の本文が null でもエラーにせず追記する" {
  export GH_ISSUE_LIST_JSON="$(issue_list_json $'42\tchore: 設定が一致していない')"
  export GH_ISSUE_VIEW_JSON="$(issue_view_json "<null>")"
