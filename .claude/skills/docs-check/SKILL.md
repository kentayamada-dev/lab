...
- **Do not remove what something depends on.** The top comment of `scripts/sync-repo-config.sh` and of each script under `.github/scripts/` is its `--help` text (CI checks none is empty), and docs may point at specific comments (`mise.toml`, `renovate.yml`, `.typos.toml`).
